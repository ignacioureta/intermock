...
export const defaultTypeToMock: {
  ...
  [ts.SyntaxKind.LiteralType]: (isFixedMode = false, node: any) => {
    return node ? node.type.literal.text : '';
  },
  // [ts.SyntaxKind.UnionType]: (isFixedMode = false, node) => {
  //   return node ? node.type.literal.text : '';
  // },
