...
function generatePrimitive(
  ...
  if (mockType) {
    ...
    if (syntaxType === ts.SyntaxKind.UnionType) {
      const types = node ? node.type.types.map((type: ts.Node) => type.kind) as ts.SyntaxKind[] : [];
      const supportedType = types.find(type => Object.values(SupportedTypes).includes(type))
      if (supportedType) {
        // @ts-ignore
        return defaultTypeToMock[supportedType as unknown as SupportedTypes](isFixedMode, node)
      }
    }
    if (Object.values(SupportedTypes).includes(syntaxType)) {
      // @ts-ignore
      return defaultTypeToMock[syntaxType](isFixedMode, node);
    }
    throw Error('unsupported mocking type');
  }
}
...
function processGenericPropertyType(
  ...
  options: Options, node?: ts.Node) {
  const mock = generatePrimitive(property, kind, options, mockType, node);
...
function processArrayPropertyType(
  ...
  for (let i = 0; i < arrayRange; i++) {
    if (isPrimitiveType) {
      ...
      // const auxNode = types[typeName] ? types[typeName].node : undefined;
      // if (auxNode) traverseInterfaceMembers(auxNode, (output[property] as Array<{}>)[i], sourceFile, options, types);
      processFile(
  ...
  const processPropertySignature = (node: ts.PropertySignature) => {
    ...
    if (node.type) {
      if (node.type.kind === ts.SyntaxKind.UnionType) {
        const types = [ts.SyntaxKind.TypeReference, ts.SyntaxKind.ArrayType, ts.SyntaxKind.FunctionType]
        // @ts-ignore
        const type = node.type.types.find(type => types.includes(type.kind));
        if (type) {
          node.type = type;
        }
    ...
    switch (kind) {
      ...
      default:
        processGenericPropertyType(
          output, property, kind as ts.SyntaxKind, '', options, node);
        break;
    }
  };
